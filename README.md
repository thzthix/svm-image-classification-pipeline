...
```env
DATA_DIR=/Users/seoha/Downloads/team_project_data
```

현재 학습 코드는 아래 파일을 기준으로 동작합니다.

- `DATA_DIR/mnist_train_small.csv`
...
```bash
python -c "from src.inference import predict_image; print(predict_image('/Users/seoha/Downloads/team_project_data/public_test_dataset/data/00001.png'))"
```
...
- 배치 추론은 아직 구현하지 않음
- API 서빙은 아직 구현하지 않음
- similarity search는 아직 구현하지 않음
- 학습 데이터 입력은 현재 `mnist_train_small.csv`에 맞춰져 있음
